...
export default function CustomTimeInput({ label, id, name, className, labelClass, iconClass, onOpenPickNewDate = false, isRequired, handleChange, selected, error, touched, showIcon = true }: CustomTimeInputProps) {
    return (
        <div className="relative">
            ...
            <DatePicker
                ...
                disableDayPicker
                format="hh:mm:ss A" plugins={[<TimePicker key={id} />]}
            />
            {showIcon && <Timer size={16} className={cn(
                "absolute right-3 top-8 text-isGray400",
                iconClass
            )} />}
            <span className={cn(
