        ...
        <MetricFrame className="flex flex-col gap-4 mt-10">
          ...
          <div className="mt-14">
            <div className="flex h-1.5 w-full rounded bg-[var(--input-bg)]">
              {/* in progress */}
              {data?.data?.my_team?.map((chi: any, idx: number) => {
      ...
      {isError ? (
        ...
        <MetricFrame className="flex flex-col gap-4 mt-10">
          <div className="flex justify-between items-center">
            ...
            <p>
              Total Tasks:{" "}
              {data?.data?.downline_team?.reduce(
                (acc: number, curr: any) => acc + curr.total,
                0
              )}
            </p>
          ...
          <div className="mt-14">
            <div className="flex h-1.5 w-full rounded bg-[var(--input-bg)]">
              {/* in progress */}
              {data?.data?.downline_team?.map((chi: any, idx: number) => {
                const { status, percentage } = chi;
                return (
                  <span
                    className={cn(
            ...
            <div className="flex justify-between items-center mt-2">
              <div className="flex gap-4 items-center mt-5 ">
                {data?.data?.downline_team?.map((chi: any) => {
                  const { status, total } = chi;
                  return (
                    <div className="flex flex-col gap-2" key={chi?.status}>
                      <div className="flex gap-2 items-center">
