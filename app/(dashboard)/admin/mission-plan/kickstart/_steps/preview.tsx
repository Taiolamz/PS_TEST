...
import { useGetFinancialYearPreviewQuery, useSaveFinancialYearMutation } from '@/redux/services/mission-plan/missionPlanApi';
...
const FinancialYearPreview = () => {
    ...
    const { data: financial_year_info, isLoading, isFetching } = useGetFinancialYearPreviewQuery({})
    const [saveFinancialYear, {isLoading: isSavingFinancialYear, reset: resetSaveFinancialYear}] = useSaveFinancialYearMutation({})
    ...
    useEffect(() => {
        ...
        if (financial_year_info) {
            setFinancialYearData({ ...financial_year_info?.data?.organization_mission_plan })
        }
    }, [financial_year_info])
                        ...
                        <div className="mt-4">
                            <div className='mb-4'>
                                <div className='bg-[var(--primary-accent-color)] p-1 px-2 flex items-center justify-between gap-5'>
                                    ...
                                    <span className='text-[var(--primary-color)] w-8 h-8 grid place-content-center rounded-full bg-[#0080801A] cursor-pointer'
                                        onClick={() => router.push(`${ADMIN.KICK_START_MISSION_PLAN}?ui=financial-year`)}
                                    ><Pencil size={15} color='var(--primary-color)' /></span>
                                </div>
                            ...
                            <div className='mb-4'>
                                <div className='bg-[var(--primary-accent-color)] p-1 px-2 flex items-center justify-between gap-5'>
                                    <span className='text-sm text-[#6E7C87] font-light'>1. Mission and Vision</span>
                                    <span className='text-[var(--primary-color)] w-8 h-8 grid place-content-center rounded-full bg-[#0080801A] cursor-pointer'
                            ...
                            <div className='mb-4'>
                                <div className='bg-[var(--primary-accent-color)] p-1 px-2 flex items-center justify-between gap-5'>
                                    <span className='text-sm text-[#6E7C87] font-light'>1. Strategic Pillars</span>
                                    <span className='text-[var(--primary-color)] w-8 h-8 grid place-content-center rounded-full bg-[var(--primary-accent-color)] cursor-pointer'
                                        onClick={() => router.push(`${ADMIN.KICK_START_MISSION_PLAN}?ui=strategic-pillar`)}
                                    ><Pencil size={15} color='var(--primary-color)' /></span>
                                </div>
                                <div className='mt-4 border rounded-[5px] p-5 bg-white'>
                                    <div className='mb-3'>
                                        {
                                            financialYearData?.strategic_pillars?.map((item: Dictionary, idx: number) => (
                                                <div key={item.id} className="mb-4">
                                                    <span className='text-xs text-[#6E7C87]'>Pillar {idx + 1}</span>
