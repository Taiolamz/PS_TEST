...
const FinancialYear = () => {
  ...
  const handleFormSubmit = async (values: Dictionary) => {
    const DATE_DIFFERENCE =
      Number(removeCharFromString(values.end_date, "-")) -
      Number(removeCharFromString(values.start_date, "-"));
    ...
    createFinancialYear(values)
      ...
      .then((payload) => {
        const obj = {
          ...formik.values,
          financial_year_id: payload?.data.organization_mission_plan?.id,
        };
        dispatch(
        ...
        <div className="mt-3 w-1/4">
          <CustomSelect
            ...
            setSelected={(selected) =>
              formik.setFieldValue("review_period", selected)
            }
            isRequired
          />
        </div>
