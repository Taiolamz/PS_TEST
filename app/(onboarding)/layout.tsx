            ...
            <aside className="w-1/4 p-4 pr-0 bg-primary text-white xl:w-2/12">
                ...
                <ul className="flex flex-col gap-2">
                    {steps.map((step, index) => (
                        <li
                            key={index}
                            className={`text-xs p-2 cursor-pointer flex items-center gap-2 mr-4 ${getCurrentStep() - 1 >= index ? "bg-white bg-opacity-[8%]" : ""
                                }`}
                            onClick={() => router.push(`${location}?ui=${ui}&step=${index + 1}`)}
                        >
